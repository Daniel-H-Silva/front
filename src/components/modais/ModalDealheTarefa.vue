<template>
  <div v-if="isModalOpen" class="modal-overlay" @click.self="closeModal">
    <div class="modal-content">
      <div class="Titulo">
        <h3>Status:</h3>
        <h3 :class="{ ativo: status === 'Aberto', fechado: status !== 'Aberto' }">{{ status }}</h3>
      </div>
      <form @submit.prevent="submitForm" class="formulario">
        ...
        <div class="campo2">
          ...
          <div>
            ...
            <select v-model="tabela.idQuadro" :disabled="!percurso.idPercurso" id="tabela" required class="preencher2">
        ...
        <div class="campo">
          ...
          <select v-model="designado.idDesignado" id="designado" required class="preencher3 opção">
            <option v-for="pessoa in pessoas" :key="pessoa.id" :value="pessoa.id" class="opção">{{ pessoa.nome }}</option>
          </select>
        </div>
        <div class="botoes">
          <button type="button" @click="closeModal" class="btn" id="voltar">Cancelar</button>
          <button type="submit" class="btn" id="criar" v-if="status === 'Aberto'">Alterar</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
import {getMembrosProjet, getGruposQuadro, updateTarefa, getInfoTarefas} from '../../services.js';

export default {
  name: 'NovoTarefaModal',
  props: ['usuarioId', 'usuarioNome', 'taskId'],
  data() {
    return {
      isModalOpen: false,
      nome: '',
  ...
  methods: {
    ...
    closeModal() {
      this.$emit('detalhe-fechado');
      this.isModalOpen = false;
      this.resetForm();
    },
    async loadTaskDetails(taskId) {
      try {
        const response = await getInfoTarefas(taskId);
        const taskDetails = response.data;
        this.nome = taskDetails.nomeTarefa;
        this.informacoes = taskDetails.infoTarefa;
        this.status = taskDetails.status;
        this.projeto = {
          nome: taskDetails.nomeProjeto,
          idProjeto: taskDetails.idProjeto,
          idGestor: taskDetails.IdGestor
        };
        this.percurso = {
          nome: taskDetails.nomePercurso,
          idPercurso: taskDetails.idPercurso
        };
        this.tabela = {
          idQuadro: taskDetails.idQuadro,
          nomeTabela: taskDetails.nomeTabela
        };
        this.designado = {
          idDesignado: taskDetails.idDesignado,
    ...
    designarParaMim() {
      if (this.pessoas.length > 0) {
        this.designado.idDesignado = this.pessoas[0].id;
      }
    },
    async submitForm() {
      try {
        const response = await updateTarefa(this.taskId,
          this.nome,
          this.informacoes,
          this.projeto.idProjeto,
          this.percurso.idPercurso,
          this.tabela.idQuadro,
          this.designado.idDesignado
        );
    ...
    resetForm() {
      this.nome = '';
      this.informacoes = '';
      this.projeto = { nome: '', idProjeto: null };
      this.percurso = { nome: '', idPercurso: null };
      this.tabela = { nomeTabela: '', idQuadro: null };
      this.designado = { nome: '', idDesignado: null };
    }
  }
};
</script>
