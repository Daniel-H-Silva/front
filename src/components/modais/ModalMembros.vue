<template>
                  ...
                  <transition :name="transitionName" mode="out-in">
                      <tbody key="page-{{ currentPage }}" class="list">
                          <tr v-for="membro in paginatedMembros" :key="membro.id" class="item">
                              ...
                              <td v-if="membro.id !== userData.id">
                                  <button class="btnRemover" @click="confirmRemove(membro)">Remover</button>
                              </td>
                              <td v-else>
                                  <button class="btnGestor">Gestor</button>
                              </td>
                          </tr>
  ...
  import {updateTarefaModalDesignado, getMembro, removerMembro} from '../../services.js';
    ...
    computed: {
        filteredMembros() {
            return this.membros.filter(membro => {
                return (
                    (!this.filterNome || membro.nome.toLowerCase().includes(this.filterNome.toLowerCase())) &&
                    (!this.filterEmail || membro.email.toLowerCase().includes(this.filterEmail.toLowerCase()))
                );
            });
        },
        paginatedMembros() {
            ...
            return this.filteredMembros.slice(start, end);
        },
        totalPages() {
            return Math.ceil(this.filteredMembros.length / this.itemsPerPage);
        }
    },
    methods: {
      async fetchMembros() {
