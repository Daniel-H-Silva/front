      ...
      <form @submit.prevent="submitForm" class="formulario">
        ...
        <div class="campo2">
          <div>
            ...
            <select v-model="projeto" @change="handleProjectChange" id="projeto" required class="preencher2">
              <option value="">Selecione um projeto</option>
              <option v-for="projeto in projetos" :key="projeto.nomeProjeto" :value="projeto">{{ projeto.nomeProjeto }}</option>
            </select>
          </div>
          <div>
            <label for="percurso" class="label">Percurso:</label>
            <select v-model="percurso" :disabled="!projeto" id="percurso" required class="preencher2" @change="updateTabelaOptions">
              <option value="">Selecione um percurso</option>
              <option v-for="percurso in percursos" :key="percurso.id" :value="percurso.id">{{ percurso.nome }}</option>
            </select>
          </div>
          <div>
            <label for="tabela" class="label">Grupo:</label>
            <select v-model="tabela" :disabled="!percurso" id="tabela" required class="preencher2">
...
import {postTarefa, getGruposQuadro, getMembrosProjet, getPercursoOptions, getTodosUsuarios} from '../../services.js';
  ...
  methods: {
    async openModal() {
      ...
      try {
        ...
        const response = getTodosUsuarios(userId);
    ...
    closeModal() {
      this.$emit('tarefa-fechado');
      this.isModalOpen = false;
      this.resetForm();
    },
    async updatePercursoOptions() {
      this.percurso= '';
      this.percursos = [];
      this.tabela = '';
      if (this.projeto) {
        try {
          const response = await getPercursoOptions(this.projeto.idProjeto);
          const percursoVigente = response.data;
          this.percursos = [{
            id: percursoVigente.idPercurso,
            nome: percursoVigente.nomePercurso}];
        } catch (error) {
          console.error('Erro ao buscar percurso do projeto:', error);
        }
      }
    },
    ...
    handleProjectChange() {
      ...
      this.updatePercursoOptions();
    },  
    async updateTabelaOptions() {
      if (this.projeto && this.percurso) {
        try {
          const response = await getGruposQuadro(this.projeto.idProjeto, this.percurso);
    ...
    async submitForm() {
      try {
        const response = await postTarefa(
          this.nome,
          this.informacoes,
